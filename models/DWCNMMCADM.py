...
class DGCNN_Grouper(nn.Module):
    ...
    def get_graph_feature(self, coor_q, x_q, coor_k, x_k):
        ...
        k = self.k
        #print(f' x_ksize, {x_k.size()}')
        #print(f' x_qsize, {x_q.size()}')
        #if x_k.dim() == 2: 
            #x_k = x_k.unsqueeze(0)
        #if x_q.dim() == 2: 
            #x_q = x_q.unsqueeze(0)    
...
class CleanNoisySeparator(nn.Module):
    def __init__(self, feature_dim=1024, num_heads=8, dropout=0.1):
        super().__init__()
        
        # ---- Clean Path ----
        self.clean_attn = MultiheadAttention(feature_dim, num_heads, dropout=dropout)
        self.clean_ffn = nn.Sequential(
            nn.Linear(feature_dim, 4 * feature_dim),
            nn.GELU(),
            nn.Linear(4 * feature_dim, feature_dim)
        )
        self.clean_norm1 = nn.LayerNorm(feature_dim)
        self.clean_norm2 = nn.LayerNorm(feature_dim)
        
        # ---- Noisy Path ----
        # Multi-scale convs to capture local/global noise
        self.noisy_convs = nn.ModuleList([
            nn.Conv1d(feature_dim, feature_dim, kernel_size=k, padding=k//2)
            for k in [1, 3, 5]
        ])
        self.noisy_merge = nn.Linear(3 * feature_dim, feature_dim)
        
        # ---- Orthogonality Regularization ----
        self.ortho_weight = 0.1  # Weight for orthogonality loss

    def forward(self, x):
        """Input: (batch, seq_len, 1024)"""
        # ---- Clean Features ----
        # Self-attention + FFN (like Transformer)
        x_clean = x.transpose(0, 1)  # (seq_len, batch, 1024)
        #print('f x_clean: {x_clean.shape}')
        attn_out, _ = self.clean_attn(x_clean, x_clean, x_clean)
        x_clean = self.clean_norm1(x_clean + attn_out)
        ffn_out = self.clean_ffn(x_clean)
        y_clean = self.clean_norm2(x_clean + ffn_out)
        y_clean = y_clean.transpose(0, 1)  # (batch, seq_len, 1024)
        
        # ---- Noisy Features ----
        x_noisy = x.transpose(1, 2)  # (batch, 1024, seq_len)
        noisy_features = [conv(x_noisy) for conv in self.noisy_convs]
        noisy_features = torch.cat(noisy_features, dim=1)  # (batch, 3*1024, seq_len)
        y_noisy = self.noisy_merge(noisy_features.transpose(1, 2))  # (batch, seq_len, 1024)
...
class ImprovedContrastiveLoss(nn.Module):
    ...
    def forward(self, y_clean, y_clean_gt, y_noisy):
        """
        Args:
            y_clean: (batch_size, seq_len, feat_dim)
            y_clean_gt: (batch_size, seq_len, feat_dim)
            y_noisy: (batch_size, seq_len, feat_dim)
        Returns:
            Scalar loss value
        """
        # Normalize features along feature dimension
        y_clean = F.normalize(y_clean, p=2, dim=-1)
        y_clean_gt = F.normalize(y_clean_gt, p=2, dim=-1)
        y_noisy = F.normalize(y_noisy, p=2, dim=-1)

        # ---- Positive Loss ----
        # Compute cosine similarities between clean and ground truth
        pos_sim = torch.sum(y_clean * y_clean_gt, dim=-1)  # (batch_size, seq_len)
        pos_loss = -torch.mean(pos_sim)  # Scalar value
        #print(f'pos_loss : {pos_loss}')

        # ---- Negative Loss ----
        # Compute all pairwise similarities between clean and noisy
        neg_sim = torch.matmul(y_clean, y_noisy.transpose(-1, -2))  # (batch_size, seq_len, seq_len)
        
        # Create mask to exclude diagonal (self-similarities)
        mask = torch.eye(y_clean.size(1), device=y_clean.device).bool()  # (seq_len, seq_len)
        
        # Apply mask and compute mean negative similarity
        neg_sim_masked = neg_sim.masked_fill(mask, float('-inf'))  # Fill diagonal with -inf
        neg_loss = torch.logsumexp(neg_sim_masked / self.temperature, dim=-1).mean()  # Scalar value
...
class DWCNET(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.nmmmodel = CleanNoisySeparator()
    ...
    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, denoised_coarse, denoised_fine, y_clean, y_noisy, clean_global_features, pred_fine = ret
        ...
        #noise loss
        # Losses
        contrastive_loss = self.contrastive_loss(y_clean, clean_global_features, y_noisy)
        #recon_loss = F.mse_loss(y_clean + y_noisy, xyz)  # Optional

        return loss_denoised, loss_recon, contrastive_loss

    def forward(self, xyz, xyz_clean):
        
        #assert contrastive_loss(zero_feats, zero_feats) == 0, "Identical features should give zero loss"
        q, coarse_point_cloud, denoise_length = self.base_model(xyz)# B M C and B M 3
        
        q_clean, coarse_point_cloud_clean, denoise_length_clean = self.base_model(xyz_clean)# B M C and B M 3
    
        B, M ,C = q.shape

        global_feature1 = self.increase_dim(q.transpose(1,2)).transpose(1,2) # B M 1024
        #print(f'global_feature1 : {global_feature1.shape}')
        #global_feature = torch.max(global_feature1, dim=1)[0] # B 1024
        
        clean_global_features = self.increase_dim(q_clean.transpose(1,2)).transpose(1,2) # B M 1024
        #print(f'clean_global_features {clean_global_features.shape}')

        ########################################################

        y_clean, y_noisy = self.nmmmodel(global_feature1)
        
        ########################################################
    
        global_feature = y_clean
        global_feature = torch.max(global_feature, dim=1)[0] # B 1024
        ...
        if self.training:
            ...
            ret = (pred_coarse, denoised_coarse, denoised_fine,y_clean, y_noisy, clean_global_features, pred_fine )
            return ret
            ...
            ret = (coarse_point_cloud, y_clean, y_noisy, clean_global_features, rebuild_points)
            return ret