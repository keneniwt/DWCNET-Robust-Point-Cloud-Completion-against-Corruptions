...
class ImprovedContrastiveLoss(nn.Module):
    ...
    def forward(self, y_clean, y_clean_gt, y_noisy):
        ...
        y_clean_gt = F.normalize(y_clean_gt, dim=-1)
        y_noisy = F.normalize(y_noisy, dim=-1)
        
        # ---- Positive Loss (clean vs ground truth) ----
        pos_sim = torch.sum(y_clean * y_clean_gt, dim=-1)  # (batch, seq_len)
        pos_loss = -torch.log(torch.exp(pos_sim / self.temperature)).mean()
        
        # ---- Negative Loss (clean vs noisy) ----
        # Intra-batch negatives: contrast across sequence positions
        neg_sim = torch.matmul(y_clean, y_noisy.transpose(-1, -2))  # (batch, seq_len, seq_len)
        neg_mask = 1 - torch.eye(y_clean.shape[1]).to(y_clean.device)  # Ignore diagonal
        neg_loss = -torch.log(1 - torch.exp(neg_sim / self.temperature) * neg_mask).mean()
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    #ConLoss = ImprovedContrastiveLoss()
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['SparseLoss', 'DenseLoss', 'ContrastiveLoss'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Completion3D' or dataset_name == 'Projected_ShapeNet':
                partial = data[0].cuda()
                #print(partial.shape)
                clean_partial = data[1].cuda()
                #print(clean_partial.shape)
                gt = data[2].cuda()
                #print(gt.shape)
            ...
            ret = base_model(partial, clean_partial)
            
            sparse_loss, dense_loss, contrastive_loss = base_model.module.get_loss(ret, gt, epoch)
         
            _loss = sparse_loss + dense_loss + contrastive_loss
            _loss.backward()
            ...
            if args.distributed:
                ...
                contrastive_loss = dist_utils.reduce_tensor(contrastive_loss, args)
                losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, contrastive_loss])
            else:
                losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, contrastive_loss])
            ...
            if train_writer is not None:
                ...
                train_writer.add_scalar('Loss/Batch/Contrastive', contrastive_loss.item() * 1000, n_itr)
        ...
        if train_writer is not None:
            ...
            train_writer.add_scalar('Loss/Epoch/Contrastive', losses.avg(2), epoch)
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    test_losses = AverageMeter(['SparseLossL1', 'SparseLossL2', 'DenseLossL1', 'DenseLossL2']) #, 'ContrastiveLoss'])
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            #Contrastive_loss = ConLoss(clean_feat, true_clean_feat,noise_feat)
            ...
            test_losses.update([sparse_loss_l1.item() * 1000, sparse_loss_l2.item() * 1000, dense_loss_l1.item() * 1000, dense_loss_l2.item() * 1000]) #Contrastive_loss.item()])
    ...
    if val_writer is not None:
        ...
        #val_writer.add_scalar('Loss/Epoch/Contrastive', test_losses.avg(4), epoch)
...
def test_net(args, config):
    ...
    ChamferDisL1 = ChamferDistanceL1()
    ChamferDisL2 = ChamferDistanceL2()
    ChamferDisL2Split = ChamferDistanceL2_split()

    test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, ChamferDisL2Split, args, config, logger=logger)

def test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, ChamferDisL2Split, args, config, logger = None):

    base_model.eval()  # set model to eval mode

    test_losses = AverageMeter(['SparseLossL1', 'SparseLossL2', 'DenseLossL1', 'DenseLossL2', 'Fidelity_loss'])# 'ContrastiveLoss'])
    test_metrics = AverageMeter(Metrics.names())
    category_metrics = dict()
    n_samples = len(test_dataloader) # bs is 1
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Projected_ShapeNet':
                ...
                dense_loss_l1 =  ChamferDisL1(dense_points, gt)
                dense_loss_l2 =  ChamferDisL2(dense_points, gt)
                fidelity_loss = ChamferDisL2Split(dense_points, gt)
                # Get just the first element of the tuple (pred->gt distance)
                fidelity_value = fidelity_loss[0] if isinstance(fidelity_loss, tuple) else fidelity_loss
                
                test_losses.update([sparse_loss_l1.item() * 1000, sparse_loss_l2.item() * 1000, dense_loss_l1.item() * 1000, dense_loss_l2.item() * 1000, fidelity_value.item() * 1000]) #, Contrastive_loss.item()])
                ...
                partial = data[0].cuda()
                print(f'partial_shape: {partial.shape}')
                #clean_partial = data[0].cuda()
                ret = base_model(partial, partial)
                dense_points = ret[-1]
